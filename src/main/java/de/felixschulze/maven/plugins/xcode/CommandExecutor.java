...
public interface CommandExecutor {
     ...
     * @param failsOnErrorOutput if true, throws an <code>ExecutionException</code> if there the compiler or executable
     *                           writes anything to the error output stream. By default, this value is true
     * @param ignoreResult       if true any resultCode from the executable is ignored.
     * @throws ExecutionException if compiler or executable writes anything to the standard error stream (provided the
     *                            failsOnErrorOutput is not false) or if the process returns a process result != 0.
     */

    void executeCommand(String executable, List<String> commands, boolean failsOnErrorOutput, boolean ignoreResult)
            throws ExecutionException;
    ...
    public static class Factory {
        ...
        public static CommandExecutor createDefaultCommmandExecutor() {
            return new CommandExecutor() {
                ...
                public void executeCommand(String executable, List<String> commands, boolean failsOnErrorOutput, boolean ignoreResult)
                        throws ExecutionException {
                    executeCommand(executable, commands, null, failsOnErrorOutput, ignoreResult);
                }
                ...
                public void executeCommand(String executable, List<String> commands, File workingDirectory,
                    ...
                    executeCommand(executable, commands, workingDirectory, failsOnErrorOutput, false);
                }

                public void executeCommand(String executable, List<String> commands, File workingDirectory,
                                           boolean failsOnErrorOutput, boolean ignoreResult)
                        throws ExecutionException {
                    if (commands == null) {
                    ...
                    try {
                        ...
                        if (logger != null) {
                            logger.debug("XCODE-040-000: Executed command: Commandline = " + commandline +
                                    ", Result = " + result);
                        } else {
                            System.out.println("XCODE-040-000: Executed command: Commandline = " + commandline +
                                    ", Result = " + result);
                        }
                        if ((failsOnErrorOutput && stdErr.hasError()) || (!ignoreResult && result != 0)) {
                            throw new ExecutionException("XCODE-040-001: Could not execute: Command = " +
                                    commandline.toString() + ", Result = " + result);
                        }
                    } catch (CommandLineException e) {
                        throw new ExecutionException(
                                "XCODE-040-002: Could not execute: Command = " + commandline.toString() + ", Error message = " + e.getMessage());
                    }
                ...
                class ErrorStreamConsumer
                    ...
                    public ErrorStreamConsumer() {
                        if (logger == null) {
                            System.out.println("XCODE-040-003: Error Log not set: Will not output error logs");
                        }
                        error = false;
                    }
